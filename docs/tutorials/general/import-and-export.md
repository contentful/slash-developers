...
Then, we must create a content model using [Contentful's Web Interface](http://www.contentful.com). With that, we download the content model and save it in a `content_model.json` file:
